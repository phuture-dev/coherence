...
class Arrays extends StaticClass
{
        ...
        foreach ($keys as $depth => $segment) {
            ...
            if (!is_string($segment) && !is_int($segment)) {
                throw new InvalidArgumentException(
                    'Invalid Argument: Array keys must be strings or integers, ' . get_debug_type($segment) . ' given'
                );
            }

            if (!array_key_exists($segment, $current)) {
                // For intermediate keys, create an array; for final key, create null
                $current[$segment] = ($depth < count($keys) - 1) ? [] : null;
            }
    ...
    public static function combine(array $keys, array $values): array
    {
        ...
        foreach ($keys as $candidateKey) {
            if (!is_int($candidateKey) && !is_string($candidateKey)) {
                throw new InvalidDataTypeException(
     ...
     * @throws \Phuture\Coherence\Exception\InvalidArgumentException When fewer than 2 arrays provided or any is empty
     */
    public static function crossJoin(array ...$arrays): array
    {
        ...
        foreach ($arrays as $arr) {
            if ($arr === []) {
                throw new InvalidArgumentException(
                    "Invalid Argument: Arrays must not be empty"
                );
            }
        }
    ...
    public static function denote(array $array, bool $strict = false): array
    {
        ...
        foreach ($keys as $key) {
            ...
            foreach ($parts as $index => $segment) {
                if ($index === $lastIndex) {
                    if ($strict && isset($temp[$segment]) && is_array($temp[$segment]) && !empty($temp[$segment])) {
     ...
     * ```
     *
     * @param array ...$arrays Two or more arrays to merge together
     * @param bool $recursive Whether to deep-merge nested arrays instead of overwriting (default: false)
     * @return array Returns a new merged array
     * @throws \Phuture\Coherence\Exception\InvalidArgumentException When fewer than 2 arrays or any is empty
     * @see \Phuture\Coherence\Arrays::collapse()
     */
    public static function merge(...$arrays): array
    {
        $recursive = self::getBoolFromArguments($arrays);
        ...
        return $recursive ? array_merge_recursive(...$arrays) : array_merge(...$arrays);
    }
    ...
    public static function remove(array &$array, string|int|array $key): void
    {
        ...
        if (is_array($current) && (is_string($lastKey) || is_int($lastKey))) {
