...
class HashTest extends TestCase
{
    ...
    public function testAlgorithmConsistency(): void
    {
        ...
        Assert::same(Hash::hash($data, false, 'md5'), Hash::md5($data));
        Assert::same(Hash::hash($data, false, 'sha1'), Hash::sha1($data));
        Assert::same(Hash::hash($data, false, 'sha256'), Hash::sha256($data));
        Assert::same(Hash::hash($data, false, 'sha512'), Hash::sha512($data));
    }
    ...
    {
        $hash = Hash::hash('Hello, World!', false, 'sha256');
        Assert::same('dffd6021bb2bd5b0af676290809ec3a53191dd81c7f70a4b28688a362182986f', $hash);
    }
    ...
    {
        Assert::exception(function () {
            Hash::hash('test', false, 'invalid_algorithm');
