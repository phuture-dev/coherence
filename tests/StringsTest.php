...
class StringsTest extends TestCase
{
    ...
    public function testEncodingParameterChunk(): void
    {
        Assert::same(['ñ', 'a', 'ñ', 'o'], Strings::chunk('ñaño', 1, 'UTF-8'));
    }

    public function testEncodingParameterContains(): void
    {
        Assert::true(Strings::contains('héllo', 'él', true, 'UTF-8'));
        Assert::false(Strings::contains('héllo', 'ell', true, 'UTF-8'));
    }

    public function testEncodingParameterDefaultMatchesExplicit(): void
    {
        $multibyte = 'héllo wörld';

        Assert::same(Strings::lower($multibyte), Strings::lower($multibyte, 'UTF-8'));
        Assert::same(Strings::upper($multibyte), Strings::upper($multibyte, 'UTF-8'));
        Assert::same(Strings::title($multibyte), Strings::title($multibyte, 'UTF-8'));
        Assert::same(Strings::length($multibyte), Strings::length($multibyte, 'UTF-8'));
        Assert::same(Strings::contains($multibyte, 'wö'), Strings::contains($multibyte, 'wö', true, 'UTF-8'));
        Assert::same(Strings::position($multibyte, 'wö'), Strings::position($multibyte, 'wö', 0, true, 'UTF-8'));
        Assert::same(Strings::slice($multibyte, 0, 5), Strings::slice($multibyte, 0, 5, 'UTF-8'));
        Assert::same(Strings::first($multibyte, 3), Strings::first($multibyte, 3, 'UTF-8'));
        Assert::same(Strings::last($multibyte, 3), Strings::last($multibyte, 3, 'UTF-8'));
        Assert::same(Strings::after($multibyte, ' '), Strings::after($multibyte, ' ', 'UTF-8'));
        Assert::same(Strings::before($multibyte, ' '), Strings::before($multibyte, ' ', 'UTF-8'));
        Assert::same(Strings::startsWith($multibyte, 'hé'), Strings::startsWith($multibyte, 'hé', 'UTF-8'));
        Assert::same(Strings::endsWith($multibyte, 'ld'), Strings::endsWith($multibyte, 'ld', 'UTF-8'));
        Assert::same(
            Strings::entityEncode('café & "résumé"'),
            Strings::entityEncode('café & "résumé"', ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8')
        );
        Assert::same(
            Strings::entityDecode('caf&eacute;'),
            Strings::entityDecode('caf&eacute;', ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8')
        );
    }

    public function testEncodingParameterFixEncoding(): void
    {
        Assert::same('valid utf-8 ñoño', Strings::fixEncoding('valid utf-8 ñoño', 'UTF-8'));
    }

    public function testEncodingParameterHamming(): void
    {
        Assert::same(2, Strings::hamming('héllo', 'hXllX', 'UTF-8'));
    }

    public function testEncodingParameterJaro(): void
    {
        Assert::same(1.0, Strings::jaro('héllo', 'héllo', 'UTF-8'));
        Assert::same(0.0, Strings::jaro('', 'héllo', 'UTF-8'));
    }

    public function testEncodingParameterLength(): void
    {
        Assert::same(4, Strings::length('ñaño', 'UTF-8'));
        Assert::same(2, Strings::length('你好', 'UTF-8'));
    }

    public function testEncodingParameterMask(): void
    {
        Assert::same('**ño', Strings::mask('ñaño', '*', 0, 2, 'UTF-8'));
    }

    public function testEncodingParameterPad(): void
    {
        Assert::same('ñaño  ', Strings::pad('ñaño', 6, ' ', PadDirection::Right, 'UTF-8'));
    }

    public function testEncodingParameterPosition(): void
    {
        Assert::same(1, Strings::position('ñaño', 'a', 0, true, 'UTF-8'));
        Assert::same(2, Strings::lastPosition('ñaño', 'ñ', 0, true, 'UTF-8'));
    }

    public function testEncodingParameterReplaceFirst(): void
    {
        Assert::same('hXllo', Strings::replaceFirst('héllo', 'é', 'X', 'UTF-8'));
    }

    public function testEncodingParameterReplaceLast(): void
    {
        Assert::same('héllX', Strings::replaceLast('héllo', 'o', 'X', 'UTF-8'));
    }

    public function testEncodingParameterSliceAndFirst(): void
    {
        Assert::same('ña', Strings::first('ñaño', 2, 'UTF-8'));
        Assert::same('ño', Strings::last('ñaño', 2, 'UTF-8'));
        Assert::same('ña', Strings::slice('ñaño', 0, 2, 'UTF-8'));
    }

    public function testEncodingParameterWordWrap(): void
    {
        $result = Strings::wordWrap('héllo wörld', 5, "\n", false, 'UTF-8');
        Assert::same("héllo\nwörld", $result);
    }
    ...
    {
        Assert::same('---hello---', Strings::pad('hello', 11, '-', PadDirection::Both));
        Assert::same('   hello   ', Strings::pad('hello', 11, ' ', PadDirection::Both));
    }

    public function testPadBothOddPaddingGoesToRight(): void
    {
        Assert::same('--hello---', Strings::pad('hello', 10, '-', PadDirection::Both));
    }

    public function testPadLeft(): void
    {
        Assert::same('     hello', Strings::pad('hello', 10, ' ', PadDirection::Left));
        Assert::same('-----hello', Strings::pad('hello', 10, '-', PadDirection::Left));
        Assert::same('00005', Strings::pad('5', 5, '0', PadDirection::Left));
    }
    ...
    public function testPadNoOpWhenAlreadyLongEnough(): void
    {
        ...
        Assert::same('hello', Strings::pad('hello', 3, ' ', PadDirection::Left));
        Assert::same('hello', Strings::pad('hello', 3, ' ', PadDirection::Both));
    }
    ...
    public function testPadRight(): void
    {
        ...
        Assert::same('hello-----', Strings::pad('hello', 10, '-'));
        Assert::same('hello-----', Strings::pad('hello', 10, '-', PadDirection::Right));
    }
    ...
    {
        Assert::same('Year: 2026, Month: April', Strings::replaceArray('Year: ?, Month: ?', '?', ['2026', 'April']));
    ...
    {
        Assert::same('hello', Strings::replaceArray('hello', '', ['world']));
    }

    public function testReplaceArrayFewerReplacementsThanOccurrences(): void
    {
        Assert::same('Year: 2026, Month: ?', Strings::replaceArray('Year: ?, Month: ?', '?', ['2026']));
    }

    public function testReplaceArrayMultibyte(): void
    {
        Assert::same('ñoño X', Strings::replaceArray('ñoño ?', '?', ['X']));
        Assert::same('Xñoño', Strings::replaceArray('helloñoño', 'hello', ['X']));
    }
