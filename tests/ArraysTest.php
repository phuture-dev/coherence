...
class ArraysTest extends TestCase
{
    ...
    public function testCrossJoinWithEmptyArrayThrows(): void
    {
        Assert::exception(
            fn () => Arrays::crossJoin([], []),
            InvalidArgumentException::class
    ...
    public function testCrossJoinWithSingleElements(): void
    {
    ...
    public function testMergeAssociativeRecursive(): void
    {
        // Recursive: duplicate scalar keys are combined into an array
        $result = Arrays::merge(['a' => 1, 'b' => 2], ['b' => 3, 'c' => 4], true);
        Assert::same(['a' => 1, 'b' => [2, 3], 'c' => 4], $result);
    }

    public function testMergeAssociativeShallow(): void
    {
        // Shallow (default): later string key overwrites earlier one
        $result = Arrays::merge(['a' => 1, 'b' => 2], ['b' => 3, 'c' => 4]);
        Assert::same(['a' => 1, 'b' => 3, 'c' => 4], $result);
    }

    public function testMergeNestedRecursive(): void
    {
        $array1 = ['db' => ['host' => 'localhost', 'port' => 3306]];
        $array2 = ['db' => ['user' => 'admin']];

        // Recursive: nested arrays are deep-merged
        $result = Arrays::merge($array1, $array2, true);
        Assert::same(['db' => ['host' => 'localhost', 'port' => 3306, 'user' => 'admin']], $result);
    }

    public function testMergeNestedShallow(): void
    {
        $array1 = ['db' => ['host' => 'localhost', 'port' => 3306]];
        $array2 = ['db' => ['user' => 'admin']];

        // Shallow: nested array under 'db' is fully replaced by the second
        $result = Arrays::merge($array1, $array2);
        Assert::same(['db' => ['user' => 'admin']], $result);
    }

    public function testMergeNumeric(): void
    {
    ...
    {
        $result = Arrays::merge(['a' => 1, 'b' => 2], ['c' => 3, 'd' => 4]);
        Assert::same(['a' => 1, 'b' => 2, 'c' => 3, 'd' => 4], $result);
    }

    public function testMergeShallowOverlappingKeys(): void
    {
        $result = Arrays::merge(['a' => 1, 'b' => 2], ['b' => 99, 'c' => 3]);
        Assert::same(['a' => 1, 'b' => 99, 'c' => 3], $result);
    }

    public function testMergeThrowsWithEmptyArray(): void
    {
        Assert::exception(
            fn () => Arrays::merge([], [1, 2]),
            InvalidArgumentException::class
